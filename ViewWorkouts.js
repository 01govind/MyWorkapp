...
export default function ViewWorkouts() {
  ...
  let workoutList = workouts.map((workout)=> {
    return (
      ...
      <td><button onClick={()=>deleteWorkout(workout.id)} className='btn btn-danger'> Remove </button></td>
      <td><button onClick={()=>startTime(workout.id)} className='btn btn-success'> Start Time </button></td>
      <td><button onClick={()=>endTime(workout.id, workout.startTime,workout.cburn)} className='btn btn-danger'> End Time </button></td>
